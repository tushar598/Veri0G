...
export function DashboardClient({
  ...
  /** Provider address pre-filled from the ?provider= query param. Triggers
   *  verification automatically on mount — no user interaction required. */
  ...
  // ── Deep-link: auto-verify once on mount when address arrives from URL ──
  const didAutoVerify = useRef(false);

  useEffect(() => {
    if (initialProvider && !didAutoVerify.current) {
      didAutoVerify.current = true;
      handleSelect(initialProvider);
    }
  ...
  return (
    <div className="min-h-screen bg-[#FDF8F5] flex flex-col">
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-4 sm:px-8 py-12">
        ...
        {/* Search — pre-filled when deep-linked */}
        <div className="mb-10">
          <ProviderSearch
